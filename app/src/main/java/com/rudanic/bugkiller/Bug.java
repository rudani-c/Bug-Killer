...
//Bug is a instant of the class Assets
// this class is for Bug activity and bugstates...
    ...
    public void birth (Canvas canvas) {
        ...
        // Check if bug is alive yet
            ...
            if (curTime - startBirthTimer >= timeToBirth) {
                ...
                if (x < Assets.roach1.getWidth())
                    x = Assets.roach1.getWidth();
                else if (x > canvas.getWidth() - Assets.roach1.getWidth())
                    x = canvas.getWidth() - Assets.roach1.getWidth();
                y = 0;
...
// anyway it is good as it make bugs movement unpredictable, so I will keep it as it is
            if(x>canvas.getWidth()/2){
                ...
                if(e) {
                    x1 += (canvas.getWidth() * elapsedTime * 0.1);
                    if (x1 == canvas.getWidth() - Assets.roach1.getWidth()) {
                        e = false;
            ...
            if(a) {  //if else taken to change 2 images one by one ,so roches looks like they are walking
                canvas.drawBitmap(Assets.roach1, x1, y, null);
                // Draw bug on screen
                a=false;
            }
            else
            {
                canvas.drawBitmap(Assets.roach2, x1, y, null);
    ...
    public void drawDead (Canvas canvas) {
        if (state == BugState.DrawDead) {
            canvas.drawBitmap(Assets.roachdead, x,  y, null);
